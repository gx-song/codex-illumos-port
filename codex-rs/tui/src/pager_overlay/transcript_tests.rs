...
#[tokio::test]
async fn transcript_overlay_reports_unsupported_link_open() -> Result<()> {
    use crossterm::event::KeyModifiers;
    use crossterm::event::MouseButton;
    use crossterm::event::MouseEvent;
    use crossterm::event::MouseEventKind;
    ...
    let mut buffer = Buffer::empty(area);
    overlay.render(area, &mut buffer);
    let status = Rect::new(/*x*/ 0, /*y*/ 6, area.width, /*height*/ 1);
    assert_snapshot!(buffer_to_text(&buffer, status).trim_end(), @"Could not open link: browser launching is unsupported on illumos and Solaris");
    Ok(())
}
