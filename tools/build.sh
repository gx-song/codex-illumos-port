...
usage() {
  cat >&2 <<'EOF'
Usage: tools/build.sh CODEX_SOURCE [RUST_TARGET]

Builds the complete upstream Codex CLI from a patched Codex source checkout.
RUST_TARGET defaults to x86_64-unknown-illumos.
...
Example:
  ...
  tools/build.sh ../codex x86_64-unknown-illumos
EOF
}
...
repo_root="$(cd "$1" && pwd)"
target="${2:-x86_64-unknown-illumos}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
strip_tool=()
if [[ ! -f "${repo_root}/codex-rs/Cargo.toml" ]]; then
  echo "Not a Codex source checkout: ${repo_root}" >&2
  exit 2
fi
...
if [[ -n "${target}" ]]; then
  ...
  output="${target_dir}/${target}/release/codex"
else
  target_dir="${CARGO_TARGET_DIR:-${repo_root}/codex-rs/target}"
  if [[ "${target_dir}" != /* ]]; then
    target_dir="${repo_root}/codex-rs/${target_dir}"
  fi
  output="${target_dir}/release/codex"
fi
